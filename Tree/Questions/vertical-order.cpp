...
class Tree
{
    ...
    public:
        ...
        pair<int, int> diameterOptimized(Node *root)
        {
            if(root)
            {
                pair<int, int> op1 = diameterOptimized(root->left);
                pair<int, int> op2 = diameterOptimized(root->right);
                int op3 = op1.second + 1 + op2.second;

                int max_diameter = max(op3, max(op1.first, op2.first));
                int maxHeight = max(op1.second, op2.second);
                pair<int, int> p = make_pair(max_diameter, maxHeight+1);
                return p;
            }
    ...
    void verticalTraversal()
    {
        ...
        while(!q.empty())
        {
            ...
            vertical[hd].push_back(p->data);
            if(p->left)
                q.push(make_pair(hd-1, p->left));
            if(p->right)
                q.push(make_pair(hd+1, p->right));
        }
...
int main()
{
    ...
    cout << tree.diameterOptimized() << endl;
